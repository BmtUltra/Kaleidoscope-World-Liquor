...
public class SoundEvents {
    ...
    private static final ResourceLocation COOL_ICE_TEA_ITEM =
            ResourceLocation.parse("kaleidoscope_world_liquor:cool_tea");
    ...
    public static void onTickDrinking(LivingEntityUseItemEvent.Tick event) {
        ...
        if (entity.level().isClientSide && isCoolIceTea(stack) && stack.getUseAnimation() == UseAnim.DRINK) {
            int usedTicks = stack.getUseDuration() - event.getDuration();
            if (usedTicks % 4 == 0) {
    ...
    private static void playCoolIceTeaSound(LivingEntity entity) {
        entity.level().playSound(
                ...
                entity.getY(),
                entity.getZ(),
                ModSounds.COOL_ICE_TEA_DRINK.get(),
                SoundSource.PLAYERS,
                1.0F,
                1.0F
        );
